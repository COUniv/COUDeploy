      ...
      <Panel shadow>
        ...
        <div class="submission-container">
          <table v-if="!getRejudgeAccess" class="submission-list">
            ...
            <tbody v-for="(submission, index) in submissions" :key="index">
              <tr>
                <td id="user-id" @click="redirectToUser(submission)">
                  {{ submission.username }}
                </td>
                <td id="problem-id" @click="redirectToProblem(submission)">
                  {{ submission.problem }}
                </td>
                <td>
                  <Tag
                    :color="toColor(submission)"
                    :checkable="checkIfAvailable(submission)"
                    v-bind:style="[
                      checkIfAvailable(submission)
                        ? { cursor: 'pointer' }
                        : { cursor: 'default' }
                    ]"
                    @on-change="redirectToDetails(submission)"
                  >
                    {{ toResult(submission) }}
                  </Tag>
                </td>
          ...
          <table v-else class="submission-list">
            ...
            <tbody v-for="(submission, idx) in submissions" :key="idx">
              <tr>
                <td id="user-id" @click="redirectToUser(submission)">
                  {{ submission.username }}
                </td>
                ...
                <td style="text-align: center;">
                  <Icon
                    ...
                    size="18"
                    class="icon-hv"
                    @click="handleRejudge(submission.id, idx)"
                  ></Icon>
                </td>
              </tr>
            </tbody>
        ...
        <Pagination
          ...
          :current.sync="page"
          style="margin: 20px 0 10px; display: flex; justify-content: center; float: none;"
        ></Pagination>
      </Panel>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getRealtimeStatus(id, index) {
      ...
      const checkStatus = () => {
        api.getSubmissionStatus(id).then(
          __ => {
            if (
              this.submissions[index] === undefined ||
              this.submissions[index].result === undefined
            ) {
              clearTimeout(this.refreshStatus);
            } else if (this.submissions[index].id !== __.data.data.id) {
              clearTimeout(this.refreshStatus);
            } else {
              api.getSafeSubmissionStatus(id).then(
                res => {
                  if (res.data.data.result === undefined) {
                    ...
                    if (this.submissions[index].id !== res.data.data.id) {
                      ...
                      this.submissions[index].statistic_info =
                        res.data.data.statistic_info;
                      if (
                        this.submissions[index].result === "7" ||
                        this.submissions[index].result === 7
                      ) {
    ...
    toMemory(submission) {
      return utils.submissionMemoryFormat(
    ...
    toResult(submission) {
      return this.$i18n.t(
        "m." + JUDGE_STATUS[submission.result].name.replace(/ /g, "_")
      );
    },
    adjustRejudgeColumn() {
      if (!this.rejudgeColumnVisible || this.rejudge_column) {
      ...
      if (this.contestID) {
        ...
        this.$router
          .push({
            ...
            params: { problemID: submission.problem }
          })
          .catch(() => {});
      }
    },
