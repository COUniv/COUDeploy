...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleLogin () {
        this.validateForm('formLogin').then(valid => {
          ...
          api.login(formData).then(res => {
            ...
            // verifying account
            api.getUserInfo(this.formLogin.username).then(res => {
              this.profile = res.data.data
              if (this.profile.user.admin_type === 'Regular User') {
                if (this.profile.user.is_email_verify === false || !this.profile.user.is_email_verify) {
                  this.goVerifyEmail()
                }
              }
            })
      ...
      goVerifyEmail () {
        this.changeModalStatus({visible: false})
        this.$router.push({name: 'apply-verify-email'})
      }
    },
    computed: {
