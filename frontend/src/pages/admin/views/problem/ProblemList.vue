      ...
      <el-table
        ...
        <el-table-column
          ...
          <div slot-scope="scope">
            <icon-btn name="Edit" icon="edit" @click.native="goEdit(scope.row, scope.row.id)"></icon-btn>
            <icon-btn v-if="contestId" name="Make Public" icon="clone"
                      @click.native="makeContestProblemPublic(scope.row, scope.row.id)"></icon-btn>
            ...
            <icon-btn icon="trash" name="Delete Problem"
                      @click.native="deleteProblem(scope.row, scope.row.id)"></icon-btn>
          </div>
        </el-table-column>
      </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goEdit (val, problemId) {
        if ((val.created_by.username === 'root') && this.$store.getters.isSuperAdmin === false) {
          this.$error('접근 권한 없음')
        } else {
      ...
      makeContestProblemPublic (val, problemID) {
        if ((val.created_by.username === 'root') && this.$store.getters.isSuperAdmin === false) {
          this.$error('접근 권한 없음')
        } else {
