...
from ..models import Problem, ProblemRuleType, ProblemTag, ProblemCategory, ProblemCategoryList
from ..serializers import (CreateContestProblemSerializer, CompileSPJSerializer,
                           ...
                           FPSProblemSerializer, ProblemCategorySerializer, CreateProblemCategorySerializer,
                           ProblemSerializer, ModifyProblemCategorySerializer, TagSerializer)
from ..utils import TEMPLATE_BASE, build_problem_template

class CreateProblemTagAPI(APIView):
...
class ModifyProblemTagAPI(APIView):
    def get(self, request):
        id = request.GET.get("id")
        name = request.GET.get("name")

        if ProblemTag.objects.filter(id = id):
...
class DeleteProblemTagAPI(APIView):
    def get(self, request):
        id = request.GET.get("id")
        
        if ProblemTag.objects.filter(id = id):
            ProblemTag.objects.filter(id = id).delete()
            return self.success()
        else:
            return self.error("Delete Failed")


class AddProblemTagAPI(APIView):
    def get(self, request):
        name = request.GET.get("name")
        if ProblemTag.objects.filter(name = name).exists():
            return self.success(name)
        else:
            return self.error("Wrong Tag")

class GetProblemTagAPI(APIView):
    def get(self, request):
        id = request.GET.get("id")
        tag = ProblemTag.objects.get(id = id)
        return self.success(TagSerializer(tag).data)

class ModifyProblemCategoryAPI(APIView):
    @validate_serializer(ModifyProblemCategorySerializer)
    def post(self, request):
        ...
        if id:
            ...
            return self.error("Wrong Category")
...
class CategoryProblemListAPI(APIView):
    def get(self, request):
        id = request.GET.get("id")
        if id:
            category = ProblemCategory.objects.get(id=id)
            category_data = ProblemCategorySerializer(category).data
        else:
            return self.error("Wrong Category")
        ...
        for problem_id in problem_id_list:
            problems.append(Problem.objects.get(id=problem_id))
        problems = ProblemAdminSerializer(problems, many=True).data
        category_data["problem_list"] = problems
        return self.success(category_data)

class DeleteProblemCategoryAPI(APIView):
    @problem_permission_required
    def get(self, request):
        id = request.GET.get("id")
        if id:
            ProblemCategory.objects.get(id=id).delete()
            return self.success()
        else:
            return self.error("Wrong Category")
...
class ProblemCategoryAPI(APIView):
    @validate_serializer(CreateProblemCategorySerializer)
    def post(self, request):
        data = request.data
        title = data["title"]
        description = data["description"]
...
class ProblemCategoryListAPI(APIView):
    @problem_permission_required
    def get(self, request):
        categories = ProblemCategory.objects.all()
        keyword = request.GET.get("keyword")
        if keyword:
            categories = categories.filter(Q(title__icontains=keyword) | Q(id__icontains=keyword))
        return self.success(self.paginate_data(request, categories, ProblemCategorySerializer))
